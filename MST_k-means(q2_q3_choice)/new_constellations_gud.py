...
class NewConstellationModel:
    ...
    def cluster_stars(self):
        ...
        # 为每个星座生成名称
        self.generate_constellation_names()
    ...
    def analyze_cluster_shape(self, cluster_id):
        """
        分析星座的形状特征，基于星座的分布或轮廓形状，优化动物形状识别
        """
        ...
        # 计算点的分布特征
        dist_matrix = spatial_distance.squareform(spatial_distance.pdist(points_2d))
        ...
        try:
            ...
            if len(hull.vertices) == 3:
                ...
                for i in range(3):
                    j = (i + 1) % 3
                    edge_length = np.linalg.norm(points_2d[hull.vertices[i]] - points_2d[hull.vertices[j]])
                    edge_lengths.append(edge_length)
                
                # 如果两边长度相近，可能是等腰三角形
                edge_lengths_sorted = sorted(edge_lengths)
                if abs(edge_lengths_sorted[0] - edge_lengths_sorted[1]) / avg_dist < 0.2:
                    return 'triangle'
                else:
                    return 'arc'  # 不规则三角形可能更接近弧形
            ...
            if len(hull.vertices) == 4:
                ...
                # 检查对边长度比例（更严格的矩形检测）
                if abs(edge_lengths[0] - edge_lengths[2]) / avg_dist < 0.2 and \
                   abs(edge_lengths[1] - edge_lengths[3]) / avg_dist < 0.2:
                    return 'square'
                else:
                    # 检查是否为梯形（动物身体形状）
                    edge_ratios = [edge_lengths[0]/edge_lengths[2], edge_lengths[1]/edge_lengths[3]]
                    if all(0.5 < ratio < 2.0 for ratio in edge_ratios):
                        return 'square'  # 近似矩形
                    else:
                        return 'arc'  # 不规则四边形
            ...
            if len(hull.vertices) >= 5:
                ...
                if len(hull.vertices) > 4:
                    ...
                    # 检查是否为对称形状（动物身体对称性）
                    if self._check_symmetry(points_2d):
                        return 'shape'  # 对称形状更适合动物命名
        except:
            pass
        
        # 4. 基于点的分布特征进一步判断
        # 计算点的分散程度（更适合动物形状的检测）
        if std_dist / avg_dist > 0.9:  # 更分散的形状
            return 'arc'  # 分散但有一定规律
    ...
    def _check_symmetry(self, points_2d):
        """
        检查点集是否具有对称性（动物身体通常具有对称性）
        """
        if len(points_2d) < 4:
            return False
        ...
        for point in centered_points:
            # 检查x轴对称
            x_reflected = np.array([-point[0], point[1]])
            # 检查y轴对称
            y_reflected = np.array([point[0], -point[1]])
            
            # 查找最近的反射点
            x_distances = np.linalg.norm(centered_points - x_reflected, axis=1)
            y_distances = np.linalg.norm(centered_points - y_reflected, axis=1)
            
            if np.min(x_distances) < 0.1:  # 容忍度
                x_symmetry += 1
            if np.min(y_distances) < 0.1:
                y_symmetry += 1
        
        # 如果超过一半的点有对称点，则认为具有对称性
        symmetry_threshold = len(points_2d) * 0.4
        return x_symmetry > symmetry_threshold or y_symmetry > symmetry_threshold
    
    def generate_constellation_names(self):
        """
        为每个星座生成动物名称，基于星座的分布或轮廓形状
        """
        ...
        shape_analysis = {}
        for cluster_id in valid_clusters:
            shape_type = self.analyze_cluster_shape(cluster_id)
            shape_analysis[cluster_id] = shape_type
            print(f"星座{cluster_id}: 形状类型 = {shape_type}")
        
        # 按形状类型分组，然后按聚类大小排序
        shape_groups = {}
        for cluster_id, shape_type in shape_analysis.items():
            if shape_type not in shape_groups:
                shape_groups[shape_type] = []
            shape_groups[shape_type].append(cluster_id)
        
        # 在每个形状组内按聚类大小排序
        for shape_type, cluster_ids in shape_groups.items():
            cluster_sizes = [(cluster_id, len(self.stars_df[self.stars_df['cluster'] == cluster_id])) 
                           for cluster_id in cluster_ids]
            cluster_sizes.sort(key=lambda x: x[1], reverse=True)
            shape_groups[shape_type] = cluster_sizes
        
        # 为每个形状类型分配动物名称
        used_names = set()
        name_counters = {shape_type: 0 for shape_type in self.ANIMAL_NAMES_BY_SHAPE.keys()}
        
        # 按形状类型优先级处理（先处理有明显特征的形状）
        shape_priority = ['line', 'triangle', 'square', 'arc', 'circle', 'cluster', 'shape']
        
        for shape_type in shape_priority:
            if shape_type not in shape_groups:
                continue
                
            cluster_sizes = shape_groups[shape_type]
            animal_names = self.ANIMAL_NAMES_BY_SHAPE[shape_type]
            
            for cluster_id, size in cluster_sizes:
                if name_counters[shape_type] < len(animal_names):
                    # 使用该形状类型对应的动物名称
                    animal_name = animal_names[name_counters[shape_type]]
                    name_counters[shape_type] += 1
                else:
                    # 如果该形状类型的名称用完，使用其他形状类型的动物名称
                    # 优先使用通用动物名称，避免"星兽"命名
                    for other_shape in shape_priority:
                        if other_shape != shape_type and name_counters[other_shape] < len(self.ANIMAL_NAMES_BY_SHAPE[other_shape]):
                            animal_name = self.ANIMAL_NAMES_BY_SHAPE[other_shape][name_counters[other_shape]]
                            name_counters[other_shape] += 1
                            break
                    else:
                        # 如果所有动物名称都用完，使用星座编号
                        animal_name = f"星座{len(used_names) + 1}"
                
                # 确保名称唯一
                if animal_name in used_names:
                    animal_name = f"{animal_name}_{cluster_id}"
                
                used_names.add(animal_name)
                self.constellation_names[cluster_id] = animal_name
                
                print(f"星座{cluster_id} (形状={shape_type}, 大小={size}): 命名为 {animal_name}")
    ...
    def calculate_cluster_scores(self):
        ...
        for cluster_id, scores in sorted(cluster_scores.items(), 
            ...
            # 使用星座名称代替ID
            constellation_name = self.constellation_names.get(cluster_id, f"星座{cluster_id + 1}")
            print(f"{constellation_name}: 总分={scores['total_score']:.2f}, "
    ...
    def visualize_constellation_single(self, cluster_id):
        ...
        # 获取怪兽特性描述
        animal_description = self.ANIMAL_DESCRIPTIONS.get(constellation_name, '神秘星兽，形态奇异，蕴含天地灵气')
        ...
        if len(cluster_stars) > 1:
            ...
            # 计算距离矩阵
            dist_matrix = spatial_distance.squareform(spatial_distance.pdist(points))
        ...
        plt.title(animal_description, color='lightblue', fontsize=12, pad=10)
        ...
        # 使用星座名称作为文件名（替换特殊字符）
        filename = constellation_name.replace(' ', '_').replace('/', '_')
        output_file = os.path.join(output_dir, f"constellation_{filename}.png")
            ...
            if cluster_id == -1:
                ...
                if len(cluster_stars) > 0:
                    ...
                    ax.text(center_x * scale, center_y * scale, center_z * scale,
                           f"{constellation_name[:2]}", fontsize=12, color=color, 
